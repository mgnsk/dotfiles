function snag -d "Pick desired files from a chosen branch"
  ...
  set branch (git for-each-ref --sort='-committerdate' --format='%(refname:short)' refs/heads | fzf --height 20%)
  # avoid doing work if branch isn't set
  if test -n "$branch"
    # use fzf to choose files that differ from current branch
    set files (git diff --relative --name-only $branch | fzf --height 20% --layout=reverse --border --multi)
  end
